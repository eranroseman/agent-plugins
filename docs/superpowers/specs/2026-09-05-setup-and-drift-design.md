...
So the skill is vendored into `software-development` at a pinned upstream ref, with a provenance header naming it, exactly as `brainstorming` is. Three changes and no more: the file-pick rule becomes "write `AGENTS.md`, and make `CLAUDE.md` a one-line `@AGENTS.md` import"; a Git section is added; and the task-reports rule is added. A drift test asserts the rest is byte-identical to upstream at the pin, which is what keeps the adaptation honest and cheap to re-vendor.
