...
| Harness requirement              | `bin/setup` requires Claude Code and treats Codex as optional; `bin/doctor` requires neither. Claude is structural: the script lives in the clone `claude plugin marketplace add` creates and reads its desired state from it (§7.2). |
