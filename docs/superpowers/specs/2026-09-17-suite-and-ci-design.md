...
That spec is therefore maintained, not frozen. `a3c797f` already amended its §4.2 when the plugin was renamed, and its message states the rule: specs move when the tree moves, plans do not. The same change corrects the two figures the rename left behind: §4.3's `3,343` and `387` bytes are `3,335` and `379`, the emitted string `3,714` bytes and `3,702` code points; §12's `3,343` is `3,335`. Its Status line gains one sentence saying §4.2 is read by a test. `tests/test-links-resolve.sh:6-9`, which calls specs "never amended after the fact", is reworded to the same rule.
