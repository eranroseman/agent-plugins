...
| 5 | Roster: `consistency-audit` and its inspector agent, `finding-duplicate-functions`, `working-with-claude-code`, `developing-claude-code-plugins`, `diagnosing-bugs` gating, `grilling` on Codex, `adhd` and `archify` carry, `rethink-audit` adaptation, the requirements and sourcing skills, prior-art search, the out-of-scope-bug mechanism. Brief and inherited context: [issue #10](https://github.com/eranroseman/agent-plugins/issues/10) | 1, and ~~2 for the mattpocock route~~ — cleared 2026-09-06: skills.sh stays, so `skillOverrides` still reaches those skills | #60, #79, #80, #81, #82, #84, #86, #88, #110, #111 |
| 6 | `harness-backup` retirement. Kept separate from 2 because it deletes rather than writes, and because it needs measured evidence on a real machine that the replacement works. Inventory and open questions: [issue #12](https://github.com/eranroseman/agent-plugins/issues/12) | 2, **and 5** (added 2026-09-05: the backup holds four authored skills symlinked live, three of which are named items on 5's roster, so it cannot retire until they have somewhere to go) | #64, #51 |
...
Order, as amended 2026-09-05: 1, then 3, then 2 (which now carries 4), then 5, then 6. Sub-project 7 runs whenever research-vault goes public. Gate results from 1 feed 2 and 3; 3 shipped 2026-09-05 and its own results feed 2.
