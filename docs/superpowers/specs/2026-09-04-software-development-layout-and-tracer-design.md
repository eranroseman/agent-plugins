...
| 6 | `harness-backup` retirement. Kept separate from 2 because it deletes rather than writes, and because it needs measured evidence on a real machine that the replacement works | 2, **and 5** (added 2026-09-05: the backup holds four authored skills symlinked live, three of which are named items on 5's roster, so it cannot retire until they have somewhere to go) | #64, #51 |
