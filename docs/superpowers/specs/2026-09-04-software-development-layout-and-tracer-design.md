...
**Scope:** the public repository `eranroseman/agent-plugins` as a marketplace hosting two plugins, the `superpowers` subset entry taken from upstream, the Codex route, and the tracer bullet that proves the layout on both CLIs.
**Not in scope:** setup automation, hook additional context redesign, drift monitoring, roster decisions, harness-backup retirement, research-vault integration. Each is its own sub-project (§11).
...
A rename is not an update: `version` gates content, not identity, so the old plugin has to be uninstalled and the new one installed on both CLIs rather than moved by `claude plugin update`. Bumped to 0.6.0 to mark the break.
...
Ship one public repository that installs, on Claude Code and Codex, a software-dev harness:

- `software-dev`: the glue plugin. Ships the narrowed `brainstorming` skill and a SessionStart hook. Depends on `sensemaking` and the `superpowers` subset entry.
- `sensemaking`: the shared plugin, installed by both `software-dev` and, later, `research-vault`. Starts with `rethink-audit`.
- `superpowers`: obra's upstream, taken as-is by a subset entry that drops `brainstorming`. No fork.
- `mattpocock/skills`: upstream, taken as a declared subset through skills.sh. Unchanged mechanism.
...
| superpowers                    | Subset entry named `superpowers`, `git-subdir` at `path: skills`, `strict: false`, pinned by `sha`, listing 13 of 14 skill directories. `brainstorming` is the one dropped.                                       |
| brainstorming                  | Vendored into `software-dev` from upstream HEAD, **name kept**, only the description changed. Visual Companion kept.                                                                                              |
| using-superpowers              | Kept in the subset entry (Codex needs `references/codex-tools.md`). Its line 30 still names `superpowers:brainstorming`; our hook additional context repoints it.                                                 |
...
│   │   ├── hooks/using-superpowers.md     (§8)
│   │   ├── hooks/working-rules.md         (§8)
...
_Amended 2026-09-04: this file is now `hooks/claude-hooks.json`, content unchanged, declared in the Claude manifest; see the [hook spec](2026-09-04-session-start-hook-design.md) §5._
...
`hooks/session-start` reads `hooks/using-superpowers.md`, JSON-escapes it, and prints `{"hookSpecificOutput": {"hookEventName": "SessionStart", "additionalContext": "..."}}`. That envelope is what Claude Code documents and what Codex requires; one output serves both CLIs. _Amended 2026-09-04: the script now reads two files, `hooks/using-superpowers.md` and `hooks/working-rules.md`, and joins them into the envelope as the [hook spec](2026-09-04-session-start-hook-design.md) §4 describes; Codex is offered no hook at all, see the [hook spec](2026-09-04-session-start-hook-design.md) §6._

Additional context for the tracer bullet: the text of upstream `obra/superpowers:skills/using-superpowers/SKILL.md` at `b36e0829`, wrapped in the same `<EXTREMELY_IMPORTANT>` frame upstream uses, with exactly one edit: line 30, `superpowers:brainstorming` becomes `software-dev:brainstorming`. Line 22 ("invoke the brainstorming skill first") is a bare-name mention and stays. `<SUBAGENT-STOP>` stays. The lean-router redesign, the admission test, and the bridge rules are sub-project 3; this additional context exists so that the tracer measures a like-for-like replacement of upstream's injection.
...
4. `claude plugin install software-dev@eranroseman`. This pulls `sensemaking` and the `superpowers` subset entry.
...
| G3 Claude hook    | Our additional context appears once at startup, `/clear`, and `/compact`; no upstream injection                                                                                                                                                                                                                                                     | Investigate; there is no design alternative, only a defect                                                                                                                                                                   |
| G4 Codex hook     | Codex either loads `hooks/hooks.json` by manifest fallback after the trust prompt (a `software-dev@eranroseman:hooks/hooks.json:session_start:0:0` entry appears under `[hooks.state]` in `~/.codex/config.toml` and the additional context appears once in a session) or does not; either outcome is recorded and sub-project 3 designs against it | None needed; the outcome is an input                                                                                                                                                                                         |
...
- The hook: `hooks/using-superpowers.md` regenerated from the clone and diffed, and `hooks/session-start`'s output parsed back to it, including a control-character case the common five escapes miss.
...
| ------------------------ | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Subset-entry superpowers | `sha` and `version` in §5.1                                                  | upstream HEAD moves past the sha (release-driven bump)                                                                                                                                                                                                                                                                                        |
...
| mattpocock subset        | `skillFolderHash` in `~/.agents/.skill-lock.json` (skills.sh records no ref) | hash differs from upstream                                                                                                                                                                                                                                                                                                                    |
...
| 2     | Setup and materialisation **merged with 4 on 2026-09-05**: cutover automation, the mattpocock adoption route, the pinned clone and Codex symlinks, environment variables, **plus everything that was sub-project 4**. This row described two things sub-project 2 then decided against, and is corrected 2026-09-06 rather than left to mislead: it delivered `bin/setup`, a deterministic script and **not a skill**, because a sampled process has no fixed point for its own dry run to assert against (its §3, §14); and it declined to make the five `harness-backup` files repository-authored templates, so neither script writes a configuration file by hand (its §3, §7.6). Design: [2026-09-05-setup-and-drift-design.md](2026-09-05-setup-and-drift-design.md) | 1                                                                                                                           | #62 and #63, #78, #95, including the five live-tree findings |
| 3     | SessionStart hook additional context: the lean router, admission test, bridge rules, Iron Laws; Codex hook posture from G4                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 1                                                                                                                           | #61                                                          |
| ~~4~~ | **Merged into 2 on 2026-09-05.** Drift: CI upstream watch (§10.4) and the local doctor (symlink targets, clone sha, unreferenced cache, unwired components). Merged because setup and the doctor act on one inventory and are one engine: the doctor is setup's dry run, and specced apart the inventory format would be designed in 2 and inherited by 4 with no say                                                                                                                                                                                                                                                                                                                                                                                                      | —                                                                                                                           | —                                                            |
| 5     | Roster **merged with 6 on 2026-09-06**: `consistency-audit` and its inspector agent, `finding-duplicate-functions`, `working-with-claude-code`, `developing-claude-code-plugins`, `diagnosing-bugs` gating, `grilling` on Codex, `adhd` and `archify` carry, `rethink-audit` adaptation, `writing-clearly-and-concisely` into `sensemaking`, the requirements and sourcing skills, prior-art search, the out-of-scope-bug mechanism — **and then `harness-backup` retirement**, which is what remains once every skill has a home. Brief: [issue #10](https://github.com/eranroseman/agent-plugins/issues/10)                                                                                                                                                              | 1, and ~~2 for the mattpocock route~~ — cleared 2026-09-06: skills.sh stays, so `skillOverrides` still reaches those skills | #60, #79, #80, #81, #82, #84, #86, #88, #110, #111           |
| ~~6~~ | **Merged into 5 on 2026-09-06.** `harness-backup` retirement. Merged because sub-project 2 shipping hollowed it out: its drift detector is replaced by `scripts/upstream-watch` and `bin/doctor`, its lockfile copy is derived from `skills.json`, and both global instruction files empty at the cutover. What remained was the four first-party skills, which is 5's own question, plus deleting a repository and moving a cron line. Speccing them apart would have meant writing this one blind. The deletion keeps its own gate inside the merged spec, requiring measured evidence on a real machine, the way sub-project 2 kept its cutover. Inventory and open questions: [issue #12](https://github.com/eranroseman/agent-plugins/issues/12)                      | —                                                                                                                           | #64, #51                                                     |
| 7     | research-vault integration: list `research-vault@eranroseman` once that repo is public; `research-vault` declares `dependencies: ["sensemaking"]` or its own marketplace adds `allowCrossMarketplaceDependenciesOn: ["eranroseman"]`. Brief: [issue #11](https://github.com/eranroseman/agent-plugins/issues/11)                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 1, and research-vault's own release gate                                                                                    | #97 stays research-vault's                                   |

Order, as amended 2026-09-06: 1, then 3, then 2 (which carries 4), then 5 (which carries 6). Sub-project 7 runs whenever research-vault goes public. Gate results from 1 feed 2 and 3; 3 shipped 2026-09-05 and its results fed 2; 2 shipped 2026-09-06 and its rulings feed 5.

### 11.2 Map positions reversed on 2026-09-04

| Map position                                                          | Now                                                                    | Primary source                                                                                                                                                                                                                                                                                      |
| --------------------------------------------------------------------- | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Fork `obra/superpowers` publicly (#75)                                | Subset entry taken from upstream, no fork                              | `amd-skills` entry in `anthropics/claude-plugins-official`: `git-subdir`, `strict: false`, 4 of 8 upstream directories listed; Claude docs on `strict` and marketplace `skills`; Codex `.codex-plugin` `skills` supplements rather than replaces (plugin-creator reference), so Codex gets symlinks |
...
| SessionStart `additionalContext` values concatenate; matching hooks run in parallel                                                                                                                                                    | `obra/superpowers-developing-for-claude-code:skills/working-with-claude-code/references/hooks.md` lines 520 and 776                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Our additional context sits in upstream's own `<EXTREMELY_IMPORTANT>` frame (§8), differing only at the line-30 edit                                                                                                                   | `obra/superpowers` `hooks/session-start` at `b36e0829`, the `session_context=` assignment; decoded `additionalContext` from upstream's own hook diffed against ours on 2026-09-04, byte-identical but for that line. Note `tests/test-hook.sh` _transcribes_ this frame rather than reading it from the clone, so a pin bump that also changed upstream's frame would leave the suite green while our injection diverged — an unwatched drift input for sub-projects 3 and 4                                                                                                                                |
...
| mattpocock ships a Claude plugin (25 skills) and no Codex manifest; 11 of 25 are model-invocable                                                                                                                                       | `gh api repos/mattpocock/skills/contents/.claude-plugin/plugin.json`; per-skill frontmatter and `mattpocock/skills:skills/*/*/agents/openai.yaml` at HEAD `6654f6b6`                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Codex loads `hooks/hooks.json` when the manifest has no `hooks` key; `{}` suppresses it; the hook-file parser accepts `timeout`, `async`, `statusMessage` and ignores `shell`                                                          | `openai/codex:codex-rs/core-plugins/src/loader.rs` line 1230 and `openai/codex:codex-rs/config/src/hook_config.rs` at openai/codex `f3f6922519fa38487c8250c2b8a670a39a2cf9ff`; upstream superpowers `obra/superpowers:tests/codex/test-marketplace-manifest.sh`. **Qualified 2026-09-04 (§14, G4):** the source reads as stated, but the fallback did not fire for our plugin on codex-cli 0.147.0 — no trust prompt, no `[hooks.state]` entry, in-session count 0. Whether the installed build differs from this sha, or the fallback has another precondition, is unresolved and belongs to sub-project 3 |
...
- The one qualified cross-reference the subset entry leaves dangling, `using-superpowers/SKILL.md:30` (opened by §14's G2 observation, sub-project 3).
...
- **Codex namespaces the symlinked skills itself, and G2's premise was wrong in our favour.** Codex's own catalog, read from the session rollout log, lists all 13 as `superpowers:<name>` with `(file: r0/<name>/SKILL.md)`, while other skills in the same `r0` root (`~/.codex/skills`) appear bare. The mechanism: Codex resolves the symlink, walks up from `skills/<name>` to the clone root, finds `.codex-plugin/plugin.json` with `"name": "superpowers"`, and namespaces from that manifest; the mattpocock skills resolve into `~/.agents/skills/`, which has no manifest above it, so they stay bare. Consequently **25 of the 26** qualified `superpowers:` cross-references name one of the 13 catalog entries and resolve natively on this route, so the concern §9.2 raises — qualified references finding only bare entries — does not arise, and **the G2 fallback recorded in §9.2 and §10.2 is not needed and should not be taken**: it would repair one reference by installing all 14 upstream skills and putting `brainstorming` on Codex twice.
  ...
  The 26th is the one §12 already singles out: `using-superpowers/SKILL.md:30` names `superpowers:brainstorming`, the skill the subset entry drops. It names no catalog entry on Codex — and none on Claude either, since the subset entry excludes it on both CLIs. §8's hook injects a _corrected copy_ of that line rather than editing the skill file, so the file's own reference dangles wherever the body is read directly, and on Codex it dangles unmitigated because G4 is IGNORED. This fails because the subset entry excludes it, not because of the namespacing mechanism G2 tested. Carried to sub-project 3 with the Codex hook posture.

- **The subset entry's exclusion survives the manifest read.** That upstream manifest declares `"skills": "./skills/"`, all 14 directories, yet the catalog holds exactly 13 `superpowers:*` entries and no `superpowers:brainstorming`. Codex enumerates membership from `~/.codex/skills`, so the symlink set decides what exists; the manifest supplies only the namespace. The manifest's `"hooks": {}` also means no upstream hook arrives by this route.
